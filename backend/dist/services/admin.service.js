...
class AdminAnalyticsService {
    static async getSystemOverview() {
        ...
        const completed = allPredictions.filter((p) => p.actual_yield !== null).length;
    ...
    static async getSystemAnalytics() {
        ...
        const completed = allPredictions.filter((p) => p.actual_yield !== null).length;
        const pending = totalPredictions - completed;
        const avgPredicted = totalPredictions > 0
            ? allPredictions.reduce((sum, p) => sum + Number(p.predicted_yield ?? 0), 0) / totalPredictions
            : 0;
        const completedPredictions = allPredictions.filter((p) => p.actual_yield !== null);
        const avgActual = completedPredictions.length > 0
            ? completedPredictions.reduce((sum, p) => sum + Number(p.actual_yield ?? 0), 0) / completedPredictions.length
            : 0;
        ...
        const cropDistribution = Object.entries(cropMap).map(([crop, count]) => ({ crop, count }));
    ...
    =============================== */
        ...
        allPredictions.forEach((p) => {
            ...
            monthlyMap[key].totalYield += Number(p.predicted_yield ?? 0);
        });
        const monthlyTrend = Object.entries(monthlyMap)
            ...
            return {
                ...
                avgYield: value.predictions > 0
                    ? Number((value.totalYield /
                        value.predictions).toFixed(2))
                    : 0,
            };
        })
