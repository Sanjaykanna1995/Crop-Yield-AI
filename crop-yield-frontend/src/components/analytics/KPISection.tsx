...
const KPISection = ({ kpis }: Props) => {
  // Mapping out the specific aesthetic for each distinct metric
  const cards = [
    { 
      label: "Predictions", 
    ...
    { 
      label: "Variance", 
      // If variance is positive, add a '+' sign for better UX
      value: kpis.variance > 0 ? `+${kpis.variance}` : kpis.variance,
      icon: Activity,
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 sm:gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
